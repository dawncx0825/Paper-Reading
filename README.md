...
论文总结统一只提交 PDF，Markdown 源文件保留在本地并加入 `.gitignore`。只有 Markdown 时，先生成 PDF，检查中文、公式、表格和图片的渲染完整性，再提交；源文件更新后同步重新生成 PDF。阅读索引指向总结 PDF，本次提交范围内已被 Git 跟踪的总结 Markdown 应取消跟踪并保留本地文件。论文原文 PDF 不替代总结。README、问答记录和复现文档照常提交。
